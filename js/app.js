(function () {
  ...
  const REBALANCE_TARGETS = [["dps", "DPS"], ["burst", "Burst"], ["damage", "Damage"], ["rotational", "Rotational DPS"]];
  ...
  function applyRebalanceLever(draft, lever, targetValue, target) {
    ...
    if (lever === "duration") {
      if (target === "burst") {
        ...
        const numerator = profile.dps * profile.duration;
  ...
  function autobalanceDraft(draft, targetValue, target, exclude) {
    const levers = target === "damage" ? ["damage", "tickCount"] : target === "rotational" ? ["damage", "tickCount", "cooldown", "endlag"] : ["damage", "tickCount", "duration"];
  ...
  function rebalanceCardChrome(view, key) {
    ...
    if (statsNode) statsNode.textContent = `Damage ${format(profile.total)} · DPS ${format(profile.dps)} · Burst ${format(profile.burst)} · Rotational DPS ${format(profile.rotationalDps)}`;
